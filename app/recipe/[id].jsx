  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          console.log("Failed to fetch favorites, status:", response.status);
          return;
        }
        
        const result = await response.json();
        
        // Fix: Extract userFavorites from the response structure
        if (result.success && result.userFavorites) {
          const isRecipeSaved = result.userFavorites.some((fav) => fav.recipeId === parseInt(recipeId));
